...
FILENAME = 'merged_sheet_info.json'
TILE_FOLDER = 'tiles'
...
def get_sheet_info(filepath):
    ...
    max_width = max([get_sheet_width(wb[name]) for name in sheet_names])
    max_height = min([get_sheet_height(wb[name]) for name in sheet_names])
    for sheet in sheet_names:
...
def get_sheet_width(ws):
    """Calculate total width of the sheet based on column dimensions."""
    total_width = 0
    for col_letter in ws.column_dimensions:
        col_width = ws.column_dimensions[col_letter].width or 8.43  # Default width
        total_width += col_width * 7.5  # Approx conversion from characters to points
    return total_width
